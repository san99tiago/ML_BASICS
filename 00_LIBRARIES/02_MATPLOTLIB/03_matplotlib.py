...
t_1 = np.linspace(0, 10, 1024)
y_1 = np.sin(t_1)
y_2 = t_1
y_3 = t_1 - t_1**3/(3*2)
y_4 = t_1 - t_1**3/(3*2) + t_1**5/(5*4*3*2)
...
# -->markersize: change size of the markers of the plot (look documentation)
ax.plot(t_1, y_1, label="sin(t) [real]", c="blue", alpha=2, linewidth=2)
ax.plot(t_1, y_2, label="Taylor[1 term]", c="yellow", alpha=0.6, linewidth=2)
ax.plot(t_1, y_3, label="Taylor[2 terms]", c="green", alpha=0.6, linewidth=2)
ax.plot(t_1, y_4, label="Taylor[3 terms]", c="red", alpha=0.6, linewidth=2)
...
ax.set_title("MACLAURIN SERIES OF <sin(t)>", color="black", fontsize=16)
...
fig.savefig(
    path_to_output_folder + "\\figure_2.pdf",
    dpi=180,
    facecolor=(0.2, 1, 1)
    )
